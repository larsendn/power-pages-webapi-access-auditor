...
import { parseAccessibleEnvironments, suggestedAllAttributesReplacement, type AllAttributesFinding, type EnvironmentTarget, type TableFinding } from './auditor'
...
import { minimumExplicitFields, resolveExplicitFields } from './approval'
...
function recordUrl(site: PowerPagesSite, entity: string, recordId: string): string {
  return physicalRecordUrl(site.environment.url, entity, recordId)
}
...
function requiresCodeChange(finding: TableFinding): boolean {
  return finding.blockers.some((blocker) => blocker.startsWith('FetchXML uses <all-attributes />') || blocker.startsWith('An OData request uses $select=*'))
}

function allAttributesSource(finding: TableFinding): AllAttributesFinding['source'] {
  return finding.blockers.some((blocker) => blocker.startsWith('An OData request uses $select=*')) ? 'odata-select' : 'fetchxml'
}

function allAttributesLabel(source: AllAttributesFinding['source']): string {
  return source === 'odata-select' ? '$select=*' : '<all-attributes />'
}
...
function App() {
  ...
  const approvalValue = (finding: FindingEntry) => {
    return resolveExplicitFields(finding.confidence, requiresCodeChange(finding), manualValues[finding.key] ?? '', finding.proposedValue, finding.table)
  }
          ...
          {stage === 'review' && (
            <section className="review-layout">
              <div className="review-list-pane">
                ...
                {reviewView === 'wildcards' && <div className="apply-bar"><span>{approved.size} wildcard change{approved.size === 1 ? '' : 's'} selected{selectedFieldsRequiredCount > 0 ? `; ${selectedFieldsRequiredCount} need fields` : ''}</span><Button icon={applying ? <Spinner size="tiny" /> : undefined} appearance="primary" disabled={approved.size === 0 || applying} aria-busy={applying} onClick={() => void applyApproved()}>{applying ? `Applying and verifying ${progress.current + 1} of ${progress.total}` : 'Apply selected and verify'}</Button></div>}
                {reviewView === 'wildcards' && (findings.length > 0 ? <>
                  <div className="wildcard-selection-toolbar">
                    ...
                    <Checkbox label={`Select all results (${findings.length})`} checked={approvalSelectionState(findings)} disabled={applying || findings.length === 0} onChange={(_, data) => toggleApprovals(findings, data.checked === true)} />
                  </div>
                  <div className="wildcard-site-groups">
                    {wildcardSiteGroups.map((group) => {
                      ...
                      <div className="wildcard-site-heading">
                        ...
                        <Checkbox label={`Select all for ${group.site.name} (${siteModelLabel(group.site.model)})`} checked={approvalSelectionState(group.findings)} disabled={applying} onChange={(_, data) => toggleApprovals(group.findings, data.checked === true)} />
                      </div>
                ...
                {reviewView === 'allAttributes' && (allAttributesFindings.length > 0 ? (
                    ...
                    {allAttributesSiteGroups.map((group) => {
                      ...
                      return <section className="wildcard-site-group" key={group.key}>
                        <div className="wildcard-site-heading">
                          <div className="wildcard-site-heading-main">
                            <Button appearance="subtle" size="small" icon={collapsed ? <ChevronRightRegular /> : <ChevronDownRegular />} aria-label={`${collapsed ? 'Expand' : 'Collapse'} ${group.site.name}`} aria-expanded={!collapsed} aria-controls={contentId} onClick={() => toggleSiteGroup(group.key)} />
                            <div><strong>{group.site.name} <Badge appearance="tint" color="informative">{siteModelLabel(group.site.model)}</Badge></strong><span>{group.site.environment.name} · {group.findings.length} all-column quer{group.findings.length === 1 ? 'y' : 'ies'}</span></div>
                          </div>
                        </div>
                        <div id={contentId} hidden={collapsed}>{group.findings.map((finding) => (
                          <button className={`finding-row ${selectedAllAttributesFindingKey === finding.key ? 'selected' : ''}`} key={finding.key} onClick={() => setSelectedAllAttributesFindingKey(finding.key)}>
                            ...
                            <span className="finding-copy"><strong>{finding.table}</strong><small>{allAttributesLabel(finding.source)} · {finding.file}:{finding.line}</small></span>
                  ...
                  <div className="empty-state"><CheckmarkCircleRegular /><h3>No all-column queries found</h3><p>No scanned request uses <code>&lt;all-attributes /&gt;</code> or <code>$select=*</code>.</p></div>
                ))}
              ...
              <aside className="finding-detail">
                {reviewView === 'wildcards' && selectedFinding ? (
                  <>
                    ...
                    <div className="manual-fields"><label htmlFor="manual-fields">Reviewed explicit fields (optional override)</label><Input id="manual-fields" value={manualValues[selectedFinding.key] ?? ''} onChange={(_, data) => updateManualValue(selectedFinding, data.value)} placeholder={approvalValue(selectedFinding)} disabled={applying} /><small>{requiresCodeChange(selectedFinding) ? 'The detected explicit field list is ready to apply. Enter an override only when fields need to be added or removed. Customer code remains unchanged and must be fixed separately.' : requiresFieldReview(selectedFinding) ? `No consumed fields were detected, so ${minimumExplicitFields(selectedFinding.table)} is the default replacement. Add every other field required by callers before applying when this table is used.` : 'Enter additional logical column names when the detected list needs adjustment.'}</small></div>
                    <div className="setting-submit-action"><Button icon={applying ? <Spinner size="tiny" /> : undefined} appearance="primary" disabled={applying} aria-busy={applying} onClick={() => void applyApproved([selectedFinding])}>{applying ? 'Applying and verifying...' : 'Apply this field setting and verify'}</Button>{!approvalValue(selectedFinding) && <small>Enter a valid comma-separated explicit field list above. Placeholder text is an example and is not submitted.</small>}</div>
                    {selectedFinding.blockers.length > 0 && (requiresCodeChange(selectedFinding) ? <><div className="blocker-panel"><strong>Separate code change required</strong><p>This request uses <code>{allAttributesLabel(allAttributesSource(selectedFinding))}</code> to request every column. You can apply the reviewed site-setting list above now. The auditor will not modify customer code; developers must replace the all-column query and test it separately.</p></div><div className="code-suggestion"><div><strong>Suggested code replacement</strong><Button appearance="subtle" size="small" icon={<CopyRegular />} onClick={() => void navigator.clipboard.writeText(suggestedAllAttributesReplacement({ proposedFields: selectedFinding.proposedFields, source: allAttributesSource(selectedFinding) }))}>Copy suggestion</Button></div><pre><code>{suggestedAllAttributesReplacement({ proposedFields: selectedFinding.proposedFields, source: allAttributesSource(selectedFinding) })}</code></pre><p>This is a starting point based on columns detected in the request. Add every column read by page rendering or business logic. After developers update and test the code, rescan to clear the All attributes finding.</p></div></> : <div className="blocker-panel"><strong>Primary ID fallback</strong><p>No consumed columns were detected for this table. The setting can be changed to <code>{minimumExplicitFields(selectedFinding.table)}</code>; add any other required fields in the optional override above before applying.</p></div>)}
                  ...
                  <>
                    <span className="step-label">ALL-COLUMN CODE EVIDENCE</span><h2>{selectedAllAttributesFinding.table}</h2><p>{selectedAllAttributesFinding.site.environment.name} / {selectedAllAttributesFinding.site.name}</p>
